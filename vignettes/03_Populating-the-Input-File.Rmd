...
vignette: >
  %\VignetteIndexEntry{Populating-the-Input-File}
...
Each reference type on DataStore has a specific set of back-end attributes. You must use a reference-type specific input.txt file (see [Generate the Input File](02_Generate-the-Input-File.html). The input files may had different columns. If your reference-type specific input.txt file lacks a column, you can safely ingore the instructions for that column. Do not edit or change the column names.
...
One suggestion is to open the .txt file in Excel and edit it in Excel. However, this can cause some issues with date formats. Please see the section below, "[Working with dates](#working-with-dates)" for how to mitigate the issue.
