...
Each reference type on DataStore has a specific set of back-end attributes. You must use a reference-type specific sheet within the DSbulkdUploadR_input.xlsx file (see [Generate the Input File](02_Generate-the-Input-File.html). The sheets within the input.xlsx may had different columns. If your reference-type specific input.xlsx sheet lacks a column, you can safely ignore the instructions for that column. Do not edit or change the column names or sheet names.
...
Titles are limited in length to ____ characters.
