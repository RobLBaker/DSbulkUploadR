...
Each reference type on DataStore has a specific set of back-end attributes. You must **use a reference-type specific sheet** within the DSbulkdUploadR_input.xlsx file (see [Generate the Input File](02_Generate-the-Input-File.html). The sheets within the input.xlsx may had different columns. If your reference-type specific input.xlsx sheet lacks a column, you can safely ignore the instructions for that column. However, you cannot leave any blanks in the .xlsx sheet for your reference type. Do not delete, edit, or change the column names or sheet names.
...
The DSbulkUploadR_input.xlsx template file also has some basic validation built in for some of the simpler columns. However, even this basic validation will not work if you copy and paste data into the sheet so copy and paste with care (why you does the validation ignore copy-paste data you ask? This is a GREAT question and I encourage you to address it directly to Microsoft....good luck).
...
One exception is that "FieldNotes" is a valid "reference type" within DSbulkUploadR but is not a reference type on DataStore. Information supplied to DSbulkUploadR as "FieldNotes" will be treated as a "GenericDocument" with the exception that the keyword "FieldNotes" will be added to the GenericDocument creatd on DataStore for triage at a later time.
...
The owner_email_list column in the input.xlsx file should contain a comma-separated list of all the owners' email addresses. Use the full email address rather than user names (e.g. fist_last@nps.gov not FMLast@nps.gov). All references require at least one owner. 

Reference owners have the ability to see reference in draft form (a particularly useful role for reviewers) as well as have full editorial abilities over the reference and can change any field or activate the reference. 

In addition to the owners in the owner_email_list field, whoever runs the `generate_references()` function will also automatically be added as an owner. If the Central Support Office is generating your references for you, they can at your request remove their names from the owner list for a given reference.
